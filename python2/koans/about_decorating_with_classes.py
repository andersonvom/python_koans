...
class AboutDecoratingWithClasses(Koan):
    ...
    def test_decorator_with_an_argument(self):
        ...
        expected = 'Increments a value by one. Kind of.'
        self.assertEqual(expected, self.count_badly.__doc__)

    def test_documentor_which_already_has_a_docstring(self):
        expected = 'Idler: Does nothing'
        self.assertEqual(expected, self.idler.__doc__)
    ...
    def test_we_can_chain_decorators(self):
        expected = "D'oh, D'oh, D'oh, D'oh"
        self.assertEqual(expected, self.homer())
